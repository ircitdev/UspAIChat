        ...
        <div className="p-5 space-y-5">
          ...
          <div className="border border-[#2d2d3f] rounded-xl overflow-hidden">
            <div className="flex items-center gap-2 px-4 py-3 bg-[#0d0d1a]">
              ...
              <span className={clsx('ml-auto text-xs px-2 py-0.5 rounded-full',
                user?.google_id ? 'bg-green-500/15 text-green-400' : 'bg-slate-700/50 text-slate-500')}>
                {user?.google_id ? 'Привязан' : 'Не привязан'}
              </span>
            </div>
            <div className="px-4 py-3">
              {user?.google_id ? (
                <p className="text-xs text-slate-500">Аккаунт Google привязан к профилю</p>
              ) : (
                <p className="text-xs text-slate-500">Войдите через Google на экране входа, чтобы привязать</p>
              )}
            </div>
          </div>
          ...
          <div className="border border-[#2d2d3f] rounded-xl overflow-hidden">
            <div className="flex items-center gap-2 px-4 py-3 bg-[#0d0d1a]">
              ...
              <span className={clsx('ml-auto text-xs px-2 py-0.5 rounded-full',
                user?.apple_id ? 'bg-green-500/15 text-green-400' : 'bg-slate-700/50 text-slate-500')}>
                {user?.apple_id ? 'Привязан' : 'Не привязан'}
              </span>
            </div>
            <div className="px-4 py-3">
              {user?.apple_id ? (
                <p className="text-xs text-slate-500">Аккаунт Apple привязан к профилю</p>
              ) : (
                <p className="text-xs text-slate-500">Войдите через Apple на экране входа, чтобы привязать</p>
              )}
            </div>
          </div>
