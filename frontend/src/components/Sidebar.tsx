  ...
  return (
    <>
      ...
      <button
        ...
        className="absolute top-1/2 -translate-y-1/2 z-50 bg-[#1e1e2e] border border-[#2d2d3f] rounded-r-lg p-1 hover:bg-[#2d2d3f] transition-all"
      ...
      <AnimatePresence initial={false}>
        {sidebarOpen && (
          <motion.aside
            ...
            className="flex flex-col bg-[#111122] border-r border-[#1e1e2e] overflow-hidden shrink-0"
          >
            {/* Header */}
            ...
            <button
              ...
              className="mx-3 mt-2 flex items-center gap-2 px-3 py-2 rounded-lg bg-[#1e1e2e] hover:bg-[#2d2d3f] text-slate-400 text-sm transition-colors"
            >
              <Search size={14} />
            ...
            <div className="flex-1 overflow-y-auto py-2 px-2">
              ...
              {pinned.length > 0 && (
                <div className="mb-1">
                  <p className="px-2 py-1 text-xs text-slate-500 uppercase tracking-wider">Pinned</p>
                  {pinned.map(conv => (
            ...
            <div className="p-3 border-t border-[#1e1e2e] space-y-1">
              {user?.role === 'admin' && (
                <button
                  ...
                  className="w-full flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-yellow-500/10 text-yellow-500 text-sm transition-colors"
                >
                  <Crown size={15} />
              ...
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-[#1e1e2e] text-slate-400 text-sm transition-colors"
              >
                <Settings size={15} />
                {t('settings')}
              ...
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#0d0d1a]">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {user?.role === 'admin' ? (
                    ...
                    <p className={`text-[10px] font-mono ${(user?.balance ?? 0) < 1 ? 'text-red-400' : 'text-green-400'}`}>
  ...
  return (
    <div
      className={clsx(
        ...
        active ? 'bg-[#2d2d3f] text-slate-100' : 'hover:bg-[#1e1e2e] text-slate-400'
      )}
      onClick={() => !isEditing && onSelect()}
      onMouseEnter={() => onHover(conv.id)}
      onMouseLeave={() => onHover(null)}
    >
      <MessageSquare size={14} className={clsx('shrink-0', active ? 'text-violet-400' : 'text-slate-500')} />
      ...
      {isEditing ? (
        <input
          ...
          className="flex-1 bg-[#0d0d1a] border border-violet-500 rounded px-1 text-sm text-slate-100 outline-none"
      ...
      {(hovered || active) && !isEditing && (
        <div className="flex items-center gap-0.5" onClick={e => e.stopPropagation()}>
          <button onClick={onPin} className="p-1 hover:text-violet-400 rounded transition-colors" title="Pin">
            <Pin size={11} className={conv.is_pinned ? 'text-violet-400' : ''} />
          </button>
          <button onClick={onRename} className="p-1 hover:text-slate-200 rounded transition-colors">
            <PencilLine size={11} />
          </button>
